/* Triela Checklists — correções de usuários, primeiro acesso universal e exclusão de perfis */
(function(){
  'use strict';
  ...
  function credentialsModal({name,username,password,code,title='Usuário criado com sucesso',subtitle='Entregue estas credenciais somente ao colaborador.'}){
    const root=document.getElementById('structureModal')||document.body.appendChild(Object.assign(document.createElement('div'),{id:'structureModal',className:'structure-modal-back'}));
    ...
    root.innerHTML=`<div class="structure-modal credential-modal-v3">
      ...
      <div class="structure-modal-body">
        <div class="credential-success-v3">
          ...
          <div class="credential-rule"><strong>Importante:</strong> este código pertence somente a este usuário e funciona uma única vez. Todo usuário novo recebe um código próprio, inclusive Gestor ou Administrador adicional.</div>
        </div>
      </div>
  ...
  window.finishTrielaCredential=function(){document.getElementById('structureModal')?.remove();setTimeout(()=>location.reload(),150);};
  ...
  window.saveOfficialUser=async function(userId=''){
    ...
    try{
      ...
      const payload={
        ...
        full_name,username,birth_date,job_title,sector,access_profile_id,store_ids,
        ...(userId?{is_active:document.getElementById('officialUserActive')?.checked!==false}:{})
      };
      if(password)payload.password=password;

      const out=await callAdmin(payload);
      if(!userId){
        if(!out.first_access_code)throw new Error('O usuário foi criado, mas o servidor não retornou o código de primeiro acesso. Gere um novo código em Editar usuário.');
        credentialsModal({name:full_name,username,password,code:out.first_access_code});
        return;
      }
      if(password&&out.first_access_code){
        credentialsModal({name:full_name,username,password,code:out.first_access_code,title:'Senha e código redefinidos',subtitle:'Use estas novas credenciais no próximo acesso do usuário.'});
        return;
      }
      document.getElementById('structureModal')?.remove();notify('Usuário atualizado com sucesso.');setTimeout(()=>location.reload(),180);
  ...
  window.resetOfficialFirstCode=async function(userId){
    if(P()?.role!=='admin')return notify('Apenas administradores podem gerar código de primeiro acesso.');
    try{
      ...
      credentialsModal({name:u?.name||'Usuário',username:u?.username||'',password:'Senha atual do usuário',code:out.first_access_code,title:'Novo código de primeiro acesso',subtitle:'O código anterior foi invalidado. Entregue este novo código ao usuário.'});
  ...
  window.removeRole=async function(id){
    ...
    const text=linked.length
      ? `Excluir definitivamente o perfil “${role.name}”? ${linked.length} usuário(s) vinculado(s) serão desativados, mas o histórico será preservado.`
      : `Excluir definitivamente o perfil “${role.name}”?`;
  ...
  function patchFirstAccessLabels(){
    const input=document.getElementById('officialFirstCode');
    if(input)input.placeholder='TRIELA-XXXXXXXXXX';
    const help=document.querySelector('#officialFirstCodeWrap .first-code-help');
    if(help)help.textContent='Todo usuário novo recebe um código individual de primeiro acesso. Use o código TRIELA-... entregue pelo administrador.';
    document.querySelectorAll('#rolesAdminList .danger-text').forEach(btn=>{if(!btn.closest('.structure-row')?.querySelector('.protected-pill'))btn.textContent='Excluir';});
  }
  const observer=new MutationObserver(patchFirstAccessLabels);observer.observe(document.documentElement,{childList:true,subtree:true});patchFirstAccessLabels();
})();
