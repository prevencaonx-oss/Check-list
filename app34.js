...
function applyBrand(){
  ...
  if(top&&!top.dataset.trielaBrandStable){
    top.dataset.trielaBrandStable='1';top.classList.add('triela-brand-fixed');
  ...
  if(bottom&&!bottom.dataset.trielaBrandStable){
    bottom.dataset.trielaBrandStable='1';bottom.classList.add('triela-powered-fixed');
...
function goToStep(index){
  ...
  const current=Math.max(0,cards.findIndex(x=>x.classList.contains('mobile-step-active'))),diff=index-current;
...
function wrapFinish(){
  ...
  window.finishRun=async function(t,answers){
    const modal=document.getElementById('runModal');if(modal?.dataset.finalizing==='1')return;
...
function stabilizeRunner(){
  if(!mobile())return;
  const modal=document.getElementById('runModal');document.body.classList.toggle('triela-checklist-open',!!modal);if(!modal)return;
  modal.querySelector('.modal')?.classList.add('triela-runner-stable');
  modal.querySelector('.modalfooter')?.classList.add('triela-runner-footer-stable');
  const finish=document.getElementById('finishBtn');if(finish){finish.classList.add('triela-finish-always');if(!finish.disabled&&!/Salvando/i.test(finish.textContent))finish.textContent='Finalizar checklist';}
  modal.querySelector('.modalbody')?.classList.add('triela-runner-body-stable');
}
